...
class R2Controller {
    ...
    getAllR2n = async (req, res) => {
        try {
            const r2n = await database_1.AppDataSource.manager.find(r2n_model_1.R2n);
    ...
    addR2n = async (req, res) => {
        try {
            const journéeAAjouter = req.body;
            const nouvelleJournée = new r2n_model_1.R2n();
            ...
            const existingEngin = await database_1.AppDataSource.getRepository(r2n_model_1.R2n).findOneBy({
    ...
    getADayR2n = async (req, res) => {
        try {
            const { date } = req.params;
            return await database_1.AppDataSource.getRepository(r2n_model_1.R2n).findOneBy({
                date: date,
            });
        }
        catch (error) {
            console.error('Erreur lors de la récupération de la journée (R2N) : ', error);
            res.status(500).json({ error: 'Erreur serveur' });
        }
    };
    // Supprime un engin
    deleteR2n = async (req, res) => {
        try {
            const journee = req.params['journee'];
            await database_1.AppDataSource.getRepository(r2n_model_1.R2n).delete({ date: journee });
            console.log('Journée du ' + journee + ' supprimée (R2N).');
            res.status(200).json(journee);
        }
        catch (error) {
            console.error('Erreur lors de la suppression de la journée (R2N) : ', error);
            res.status(500).json({ error: 'Erreur serveur' });
        }
    };
}
exports.R2Controller = R2Controller;
